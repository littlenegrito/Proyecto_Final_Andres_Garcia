...
Nodo* crearEventoMinijuego(int id, const char* nombre, const char* descripcion, Player stats, Enemy *enemigo) {
    ...
    nuevoNodo->stats = stats;
    nuevoNodo->level = 0;
    nuevoNodo->condiciones = NULL;
    nuevoNodo->isMinigame = 1;
    nuevoNodo->enemy = enemigo;
    nuevoNodo->firstChild = NULL;
    nuevoNodo->nextSibling = NULL;
    return nuevoNodo;
}
...
void agregarHijo(Nodo* nodoPadre, Nodo* nodoHijo) {
    if (nodoPadre->firstChild == NULL) {
        nodoPadre->firstChild = nodoHijo;
    } else {
        Nodo* actual = nodoPadre->firstChild;
        while (actual->nextSibling != NULL) {
            actual = actual->nextSibling;
        }
        actual->nextSibling = nodoHijo;
    }
}
...
Nodo* buscarNodo(Nodo* raiz, int id) {
    ...
    Nodo* resultado = buscarNodo(raiz->firstChild, id);
    if (resultado != NULL) return resultado;

    return buscarNodo(raiz->nextSibling, id);
}
...
void aplicarEfectos(Nodo *nodo, Player *jugador, List *inventario) {
    ...
    if(jugador->health <= 0) printf("Game Over");
...
void crearNodosHistoria(ArbolHistoria *arbol, Player jugador, List *inventario) {
    ...
    agregarHijo(arbol->root, startWait);
     ...
     //Enemy* enemigoGoblin = crearEnemigo("Goblin", 50, 2);
     //Nodo* nodoConBatalla = crearEventoMinijuego(75, "Encuentro con el enemigo", "Te encuentras con un feroz enemigo y debes luchar", statsDefault, enemigoGoblin);
    //agregarHijo(arbol->root, nodoConBatalla);
